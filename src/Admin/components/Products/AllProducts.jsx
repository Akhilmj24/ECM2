...
import { HandlerDelete,HandlerEdit } from "../../utils/Image/ServiceHandler/ServiceHandler";
import { toast } from "react-toastify";
import Loading from "../../../pages/Loading/Loading";
import { ConfirmDialog } from "primereact/confirmdialog";

const AllProducts = ({ setToggleMenu, setproductId }) => {
  const { data, error, loading, reFetch } = useFetch("/product");
  ...
  const edithandler = (id) => {
    if (id) {
      // setproductId(id);
      setToggleMenu("EditProduct");
    }
    HandlerEdit(`/product/find/${id}`)
      .then(
        (res) =>
  ...
  return (
    <div className="allProductConatiner">
        ...
        <tbody>
          {data.map((product, index) => (
            <tr key={product._id}>
              ...
              <td>{product.price}</td>
              <td>
                <Image src={product.image} alt="Image" width="35" preview />
              </td>
              <td>
                <button>
                  <svg
